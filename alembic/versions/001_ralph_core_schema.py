...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('secrets_provider', postgresql.ENUM('railway', 'aws_secrets_manager', 'gcp_secret_manager', 'vault', 'none', name='secrets_provider'), nullable=False, server_default='railway'),
        sa.Column('db_connection_ref', sa.String(256), nullable=True),
        sa.Column('db_context_mode', postgresql.ENUM('none', 'metadata_only', 'readonly', name='db_context_mode'), nullable=False, server_default='metadata_only'),
    ...
    op.create_table(
        ...
        sa.Column('build_type', postgresql.ENUM('PLAN', 'CODE', name='build_type'), nullable=False, server_default='CODE'),
        ...
        sa.Column('builder_signal', postgresql.ENUM('READY_FOR_REVIEW', 'NEEDS_WORK', 'DEPLOYED', name='builder_signal'), nullable=False, server_default='READY_FOR_REVIEW'),
        sa.Column('builder_notes', postgresql.JSONB, nullable=True),
        sa.Column('inspection_status', postgresql.ENUM('PENDING', 'PASSED', 'FAILED', name='inspection_status'), nullable=False, server_default='PENDING', index=True),
    ...
    op.create_table(
        'ralph_review_queue',
        sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True, server_default=sa.text('gen_random_uuid()')),
        sa.Column('queue_type', postgresql.ENUM('PLAN', 'CODE', name='review_queue_type'), nullable=False, index=True),
        ...
        sa.Column('status', postgresql.ENUM('PENDING', 'DISPATCHED', 'COMPLETED', 'FAILED', name='review_queue_status'), nullable=False, server_default='PENDING', index=True),
    ...
    op.create_table(
        ...
        sa.Column('status', postgresql.ENUM('PENDING', 'IN_PROGRESS', 'COMPLETED', name='revision_status'), nullable=False, server_default='PENDING'),
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
    )
