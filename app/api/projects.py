...
async def list_projects(
    ...
    return [
        ProjectResponse(
            ...
            db_connection_ref=row['db_connection_ref'],
            db_context_mode=DBContextMode(row['db_context_mode']),
            allowed_schemas=json.loads(row['allowed_schemas']) if row['allowed_schemas'] else None,
            allowed_tables=json.loads(row['allowed_tables']) if row['allowed_tables'] else None,
            pii_fields=json.loads(row['pii_fields']) if row['pii_fields'] else None,
            created_by=row['created_by'],
            created_at=row['created_at'],
            updated_at=row['updated_at']
        )
